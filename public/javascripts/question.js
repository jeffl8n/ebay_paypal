...
$scope.formData.catagory = 'Select an answer'
    ...
    $http.get('/api/questions/'+qid)
        ...
        $scope.catagorySelected = function(catagory){
                       $scope.cat_selected = true
            $scope.formData.catagory = catagory
        }

        $scope.catagoryChange = function(catagory){
 
            $http.get('/api/responses/qid/'+qid+'/'+$scope.formData.catagory)
        .success(function(data) {
